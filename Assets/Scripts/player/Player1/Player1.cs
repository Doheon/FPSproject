...
public class Player1 : PlayerStatus
{
    ...
    IEnumerator Skill1Coroutine(){
        ...
        skill1Animator.SetBool("skillOn", true);

        bulletConsume = 0;
        autoAimMode = true;

        while(Time.time < _time + skill1DurTime){
            skill1Slider.value = (skill1DurTime - Time.time + _time)/skill1DurTime;
            yield return null;
        }

        skill1Animator.SetBool("skillOn", false);
